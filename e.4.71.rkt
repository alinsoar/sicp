...
(define (simple-query query-pattern frame-stream)
  (S:flatmap
   (lambda (frame)
     ...
     ;; MODIFIED SYSTEM
     (S:append (find-assertions query-pattern frame)
...
(module+ test
  ...
  (initialize-data-base mickey-data-base)

  (infinite-loop (lambda ()
                   (test/ '((COMM "Modified interpreter")
                            (married Mickey ?who) ) ) )
                 "loop in query system without lazy streams"
                 .1
                 "loop")

  (q:initialize-data-base mickey-data-base)

  (infinite-loop (lambda ()
                   (q:test/ '(
                              (COMM "Original query interpreter")
                              (married Mickey ?who)
                              ) ) )
                 "loop in query system without lazy streams"
                 .0001
