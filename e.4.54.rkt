...
(module+ test
  "==================== QUEENS USING ``REQUIRE`` AS A SPECIAL FORM"
  ...
  (amb-test
   '((define (distinct? l)
     ...
     (define (queens)
       (let ((q1 (rows)))
         (let ((q2 (rows)))
           (require/sp (check q2 (list q1)))
           (let ((q3 (rows)))
             (require/sp (check q3 (list q1 q2)))
             (let ((q4 (rows)))
               (require/sp (check q4 (list q1 q2 q3)))
               (let ((q5 (rows)))
                 (require/sp (check q5 (list q1 q2 q3 q4)))
                 (let ((q6 (rows)))
                   (require/sp (check q6 (list q1 q2 q3 q4 q5)))
                   (let ((q7 (rows)))
                     (require/sp (check q7 (list q1 q2 q3 q4 q5 q6)))
                     (let ((q8 (rows)))
                       (require/sp (check q8 (list q1 q2 q3 q4 q5 q6 q7)))
