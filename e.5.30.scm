...
;; attempt to access an unbound variable
;; attempt to divide by zero
;; attempt to extract the car of a symbol
...
(define (lookup-variable-value var env)
  (define (env-loop env)
    ...
    (if (eq? env the-empty-environment)
        ;; one supposes 'unbound' is not a valid value for a variable.
        ;; In a real system this error symbol must be uniquely tagged
        ;; as non-valid value.
        'unbound
...
(define car?
  (lambda (proc)
    (eq? (primitive-symbol proc) 'car)))
(define division?
  (lambda (proc)
    (eq? (primitive-symbol proc) '/)))
...
primitive-apply
;;; car and division are primitive procedures.
        (test (op car?) (reg proc))
        (branch (label check-car-application))
        (test (op division?) (reg proc))
        (branch (label check-division-by-zero))
primitive-apply0
...
check-car-application
        (assign val (op first-argument) (reg argl))
        (test (op pair?) (reg val))
        (assign flag (op not) (reg flag))
        (test (op true?) (reg flag))
        (branch (label car-applied-on-non-pair))
        (goto (label primitive-apply0))
check-division-by-zero
        (assign val (op second-argument) (reg argl))
        (test (op zero?) (reg val))
        (test (op true?) (reg flag))
        (branch (label division-by-zero))
        (goto (label primitive-apply0))

car-applied-on-non-pair
        (assign val (const car-applied-on-non-pair))
        (goto (label signal-error))
