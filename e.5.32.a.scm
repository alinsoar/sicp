...
(define debug/msg
  (lambda (x)
    (display "operator is a symbol. no need to save all the registers.")
    (display x)
    (newline)))
...
;;;  THIS IS MODIFIED for this exercise
;;; --------------------------------------------------------------------------------
ev-application
        (save continue)
        ...
        (goto (label ev-appl-operator-non-symbol))
ev-appl-operator-symbol
        (perform (op dbg) (reg exp))
        ;;  here we can evaluate the operator from EXP without saving
        ;;  the ENV register and one can jump to ev-appl-did-operator.
ev-appl-operator-non-symbol
        (save env)
        (save unev)
        ;; (assign exp (op operator) (reg exp))
