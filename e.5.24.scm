...
;;; --------------------------------------------------------------------------------
'(
bad-else-conditional
        (assign val (const cond-else-error))
...
cond-loop
        ...
        (test (op last-exp?) (reg unev))
        (branch (label last-cond-expression))
        ;; error when the `else` clause isn't the last one
        (test (op cond-else-clause?) (reg val))
        (branch (label bad-else-conditional))

        ;; test predicate
        (assign exp (op cond-predicate) (reg val))
        (save unev)
        (assign continue (label cond-predicate-decision))
        (goto (label eval-dispatch))
cond-predicate-decision
        (restore unev)
        (test (op true?) (reg val))
        (branch (label evaluate-cond-true-clause))

        (assign unev (op rest-exps) (reg unev))
        (goto (label cond-loop))
last-cond-expression
        ;; else-clause
        (test (op cond-else-clause?) (reg val))
        (branch (label conditional-else))

        ;; test predicate -- the same as the previous test
        (assign continue (label cond-last-predicate-decision))
        (assign exp (op cond-predicate) (reg val))
        (save unev)
        (goto (label eval-dispatch))
cond-last-predicate-decision
        (restore unev)
        (test (op true?) (reg val))
        (branch (label evaluate-cond-true-clause))

        ;; false-branch -- return 'cond:none
        (assign val (const cond:none))
        (restore continue)
        (goto (reg continue))
evaluate-cond-true-clause
        (assign val (op first-exp) (reg unev))
conditional-else
        (assign unev (op cond-actions) (reg val))
        ;; continue was saved at the beginning of the cond-loop
        (goto (label ev-sequence))
)
